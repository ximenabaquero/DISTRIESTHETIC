    ...
    <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 space-y-4">
      ...
      >
        ...
        <div>
          ...
          <p className="text-sm font-medium text-gray-700 group-hover:text-blue-600 transition-colors">{telefono}</p>
        </div>
      </a>
      ...
      <a href={`mailto:${email}`} className="flex items-center gap-3 group">
        ...
        <div>
          ...
          <p className="text-sm font-medium text-gray-700 group-hover:text-blue-600 transition-colors">{email}</p>
        </div>
      </a>
      ...
      {items.map((item) => (
        <div key={item.label} className="flex items-start gap-3">
          <span className="w-8 h-8 rounded-lg bg-blue-50 flex items-center justify-center flex-shrink-0 text-blue-600 mt-0.5">
            {item.icon}
          </span>
          <div>
